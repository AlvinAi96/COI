...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, text_b=None, label=None):
        ...
        self.text_a = text_a # 新闻正文
        self.text_b = text_b # 新闻标题
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,split_num,
    ...
    for example_index, example in enumerate(examples):
        ...
        # 将正文拆分为几份？这里我们split_num=1，即正文不会被分割
...
def _truncate_seq_pair(tokens_a, tokens_b, max_length):
    ...
    while True:
        # 现序列总长度=新闻正文长度+新闻标题长度
        total_length = len(tokens_a) + len(tokens_b)
        # 如果现序列总长度<=最大要求长度，则不修改
        # 如果现序列总长度>最大要求长度且新闻正文长度>新闻标题长度，则修剪正文长度
        # 如果现序列总长度>最大要求长度且新闻正文长度<新闻标题长度，则修剪标题长度
